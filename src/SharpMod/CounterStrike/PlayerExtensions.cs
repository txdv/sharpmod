...
namespace SharpMod.CounterStrike
{
	...
	public static class PlayerExtensions
	{
		...
		{
			if (value) {
				player.MakeVIP(updateModel, updateScoreBoard);
			} else {
				player.UnmakeVIP(updateModel, updateScoreBoard, (InternalModels)(new Random().Next(4)));
			}
		}
		...
		{
			if (player.IsDead) {
				return ScoreAttribute.Dead;
			}
			// TODO: maybe check if model is equal VIP too?
			if (player.GetVIP()) {
				return ScoreAttribute.VIP;
			}
			// TODO: add bomb check
