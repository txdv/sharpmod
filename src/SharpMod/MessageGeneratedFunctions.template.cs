...
<% end %>
	...
	public static partial class MessageFunctions
	{
		...
		<% messages.each do |message| %>
		#region <%= message.name %>
		public static void Send<%= message.name %>Message(MessageDestination destination, IntPtr floatValue, IntPtr playerEntity<% if message.arg_array.size > 0 then %>, <% end %><%= message.function_arguments %>)
		{
	...
	public partial class Player
	{
		...
		public static void Send<%= message.name %>Message(IntPtr floatValue, <%= message.name %>Message val)
		{
			foreach (Player player in Players) {
				Message.Send<%= message.name%>Message(player, floatValue, val);
			}
		}
		...
		{
			foreach (Player player in Players) {
				Message.Send<%= message.name%>Message(player, val);
			}
		}
		...
		{
			foreach (Player player in Players) {
				Message.Send<%= message.name%>Message(player, );
			}
		}
		<% end %>
	}
